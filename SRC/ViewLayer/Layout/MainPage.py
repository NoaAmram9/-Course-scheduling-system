...
#         # === Treeview לכל הקורסים ===
#         columns = ("Code", "Name", "Instructor", "Lectures", "Exercises", "Labs")
#         self.tree_all = ttk.Treeview(self.window, columns=columns, show="headings", selectmode="extended")
#         for col in columns:
#             self.tree_all.heading(col, text=col)
#             self.tree_all.column(col, width=150, anchor="center")
#         self.tree_all.pack(side=tk.RIGHT, padx=10, pady=10, fill=tk.BOTH, expand=True)
#         self.tree_all.bind("<Double-1>", self.add_course_from_click)


#         # === Treeview לקורסים שנבחרו ===
#         self.tree_selected = ttk.Treeview(self.window, columns=columns, show="headings", selectmode="browse")
#         for col in columns:
#             self.tree_selected.heading(col, text=col)
#             self.tree_selected.column(col, width=150, anchor="center")
#         self.tree_selected.pack(side=tk.LEFT, padx=10, pady=10, fill=tk.BOTH, expand=True)

#         # === כפתורים ===
#         self.load_button = tk.Button(self.window, text="Load Courses", command=self.load_courses)
#         self.load_button.pack(pady=5)

#         self.add_button = tk.Button(self.window, text="Add Selected Courses", command=self.add_selected_courses)
#         self.add_button.pack(pady=5)

#         # === Bindings ===
#         self.tree_selected.bind("<Double-1>", self.remove_course)
        
#          # === טעינה מיידית של הקורסים ===
#         self.load_courses()  
...
#         if len(self.selected_course_ids) + len(selected) > self.max_courses:
#             messagebox.showwarning("Limit Reached", f"You can select up to {self.max_courses} courses only.")
#             return

#         for course_code in selected:
#             if course_code not in self.selected_course_ids:
#                 self.selected_course_ids.add(course_code)
#                 values = self.tree_all.item(course_code)["values"]
#                 self.tree_selected.insert("", tk.END, iid=course_code, values=values)
#                 self.tree_all.item(course_code, tags=("selected",))
#                 self.tree_all.tag_configure("selected", background="#D0D0D0")
#     def add_course_from_click(self, event):
#      item = self.tree_all.identify_row(event.y)
#      if item and item not in self.selected_course_ids:
...
#         self.tree_selected.insert("", tk.END, iid=item, values=values)
#         self.tree_all.item(item, tags=("selected",))
#         self.tree_all.tag_configure("selected", background="#D0D0D0")
...
#             self.tree_all.item(item, tags=())
#             self.tree_all.tag_configure("selected", background="")
            
#     def get_selected_courses(self):
#      selected_courses = []
#      for item in self.tree_selected.get_children():
#         course_code = self.tree_selected.item(item)["iid"]
#         selected_courses.append(self.course_map.get(course_code))
